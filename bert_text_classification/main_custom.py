...
from config import Config_multi
from preprocess_custom import DataProcessor, get_time_dif
from transformers import BertConfig, BertTokenizer, BertForSequenceClassification
from transformers import AutoConfig, AutoTokenizer, AutoModelForSequenceClassification, AutoModel
from BulidCustomModel import CustomModel
import warnings
...
def main():
    set_seed(args.seed)
    config = Config_multi(args.data_dir)
    tokenizer = AutoTokenizer.from_pretrained(args.pretrained_bert_dir)
    model = AutoModel.from_pretrained(args.pretrained_bert_dir)
    model = CustomModel(model, labeltwo=19, labelthree=92)
    print(model)
    ...
    if args.mode == "train":
        ...
        test_iterator = DataProcessor(config.test_file, config.device, tokenizer, config.batch_size, config.max_seq_len, args.seed)
        time_dif = get_time_dif(start_time)
        print("time usage:", time_dif)

        # train
        train_multitask(model, config, train_iterator, dev_iterator, test_iterator)
